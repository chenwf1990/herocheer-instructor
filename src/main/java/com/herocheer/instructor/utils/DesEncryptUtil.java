...
public class DesEncryptUtil {
     ...
     *
     * @param toEncryptData 需要加密的数据
     * @param keyData       密钥数据内容, 使用该数据的byte[]缓冲区前 8 个字节作为密钥
     ...
     *
     * @param key            密钥
     * @param toEncryptData  要加密的数据
     * @param transformation 转换的名称，例如 DES/CBC/PKCS5Padding。有关标准转换名称的信息，请参见 Java Cryptography Architecture Reference Guide 的附录 A。
     * @return 加密后的数据
     * @creator TRS
     */
     ...
     *
     * @param key            密钥
     * @param cipherMode     此 Cipher的操作模式（为以下之一：ENCRYPT_MODE、DECRYPT_MODE、WRAP_MODE 或 UNWRAP_MODE）
     * @param transformation 转换的名称，例如 DES/CBC/PKCS5Padding。有关标准转换名称的信息，请参见 Java Cryptography Architecture Reference Guide 的附录 A。
     * @return
     * @creator TRS
     */
     ...
     * 默认转换模式transformation为：DES/ECB/PKCS5Padding
     *
     * @param toDecryptData 要解密的数据
     * @param keyData       DES密钥数据，使用该数据的byte[]缓冲区前 8 个字节作为密钥
     ...
     * 根据指定的算法名称和密钥规范获取密钥
     *
     * @param algorithm 所请求的秘密密钥算法的标准名称, 如"DES"。有关标准算法名称的信息，请参阅 Java Cryptography Architecture Reference Guide 中的附录 A。
     ...
     *
     * @throws RuntimeException 当给定十六进制字符串的长度为奇数时或给定字符串包含非十六进制字符.
     * @see #toString(byte[])
     */
